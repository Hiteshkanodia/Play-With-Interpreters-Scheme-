  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (let ((parameters (procedure-parameters procedure)))
           (if (no-lazy? parameters)
                (eval-sequence (procedure-body procedure)
                               (extend-environment parameters (list-of-arg-values arguments env) (procedure-environment procedure)))
                (eval-sequence (procedure-body procedure)
                               (extend-environment (to-normal-parameters parameters) (delayed-specific-arg-values arguments parameters env) (procedure-environment procedure))))))
...
(define (delayed-specific-arg-values arguments parameters env)
  (map (lambda (arg par)
         (if (not (list? par))
...
(define (no-lazy? parameters)
  (null? (filter (lambda (x) (and (list? x) (eq? 'lazy (car x)))) parameters)))
